    ...
    def judge(self, users, items):
        ...
        for metric in self.metrics:
            ...
            for i in range(len(items)):
                results[metric] += f(items[i], test_pos=self.test_dic[users[i]],
                                     num_test_pos=len(self.test_dic[users[i]]), model=self.model)
        return results
...
class Metrics(object):
    ...
    def ndcg(items, **kwargs):
        num = len(items)
        discount = np.log2(np.array(range(num)) + 2)

        test_pos = kwargs['test_pos']
        num_test_pos = kwargs['num_test_pos']
        res = np.isin(items, test_pos)
        rdcg = (res / discount).sum()

        if num_test_pos >= num:
            res_ideal = np.ones(num)
        else:
            res_ideal = np.zeros(num)
            res_ideal[:num_test_pos] = 1
        idcg = (res_ideal / discount).sum()
