import logging
from GraphAU.utils.EarlyStop import EarlyStoppingCriterion
import torch
import numpy as np
import random
from tqdm import tqdm
import dgl
from GraphAU.models.models import LightGCN, MF, DirectAU_MF, GraphAU, NGCF
import os
...
def choose_model(args, dataloader):
    if args.model == 'lightgcn':
        return LightGCN(args, dataloader)
    if args.model == 'ngcf':
        return NGCF(args, dataloader)
    if args.model =='mf':
        return MF(args, dataloader)
    if args.model == 'directau_mf':
        return DirectAU_MF(args, dataloader)
    if args.model == 'graphau':
        return GraphAU(args, dataloader)
...
def config(args):
    ...
    path = f"{args.dataset}_model_{args.model}_lr_{args.lr}_embed_size_{args.embed_size}_batch_size_{args.batch_size}_weight_decay_{args.weight_decay}_layers_{args.layers}_neg_number_{args.neg_number}_seed_{args.seed}_k_{args.k}_sigma_{args.sigma}_gamma_{args.gamma}_cluster_num_{args.cluster_num}_beta_class_{args.beta_class}_gamma_au_{args.gamma_au}_alpha_n_{args.alpha_n}_decaying_base_{args.decaying_base}"
