...
class SBGNNUpdate(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.linear1 = nn.Linear(3 * args.embed_size, args.embed_size)
        self.dropout = nn.Dropout(0.5)
        self.act = nn.ReLU()
        self.linear2 = nn.Linear(args.embed_size, args.embed_size)

    def forward(self, user, item):
        user = self.linear2(self.act(self.dropout(self.linear1(user))))
        item = self.linear2(self.act(self.dropout(self.linear1(item))))
        return user, item
...
class LinearTransformLayer(nn.Module):
    ...
    def aggregation(self, edges):
        feat_src = edges.src['h']
        message = self.linear(feat_src)
        return {'m': message}
    ...
    def forward(self, graph, h, etype):
        with graph.local_scope():
            ...
            graph.nodes[src].data['h'] = feat_src
            graph.update_all(self.aggregation, fn.mean(msg='m', out='h'), etype=etype)
...
class NGCFLayer(nn.Module):
    def __init__(self, args):
        ...
        self.W1 = nn.Linear(self.hid_dim, self.hid_dim, bias=True)
        self.W2 = nn.Linear(self.hid_dim, self.hid_dim, bias=True)

        self.leaky_relu = nn.LeakyReLU(0.2)
        self.dropout = nn.Dropout(args.dropout)

        th.nn.init.xavier_uniform_(self.W1.weight)
        th.nn.init.constant_(self.W1.bias, 0)
        th.nn.init.xavier_uniform_(self.W2.weight)
        th.nn.init.constant_(self.W2.bias, 0)

    def reduction(self, nodes):
        feat_src = nodes.data['h'].unsqueeze(1)
        messages = self.W1(feat_src) + self.W2(feat_src * nodes.mailbox['m'])
        return {'h': messages.sum(dim=1)}
    ...
    def forward(self, graph, h, etype):
        with graph.local_scope():
            ...
            graph.nodes[src].data['h'] = feat_src
            graph.nodes[dst].data['h'] = feat_dst
            graph.update_all(aggregate_fn, self.reduction, etype=etype)
...
class FilterGNNLayer(nn.Module):
    def __init__(self, args):
        super().__init__()
        # self.weight = nn.Parameter(th.randn(args.embed_size))
        self.linear = nn.Linear(args.embed_size, args.embed_size)
    ...
    def filter_aggregation(self, edges):
        feat_src = edges.src['h']
        feat_dst = edges.dst['h']
        feat_concat = th.cat([feat_src, feat_dst], dim=1)
        # message = feat_src * self.weight
        message = self.linear(feat_src)
        return {'m': message}
    ...
    def forward(self, graph, h, etype):
        with graph.local_scope():
            ...
            graph.nodes[src].data['h'] = feat_src
            graph.nodes[dst].data['h'] = feat_dst
            graph.update_all(self.filter_aggregation, fn.sum(msg='m', out='h'), etype=etype)
...
class SubLightGCNLayer(nn.Module):
    ...
    def submodular_selection_cluster(self, nodes):
        ...
        for i in range(self.k):
            ...
            mask = cluster[th.arange(batch_num, device=device), selected].unsqueeze(-1).repeat_interleave(neighbor_num,
                                                                                                          dim=-1).transpose(
                1, 2)
            mask = mask.bool()
            cluster[mask] -= 1
